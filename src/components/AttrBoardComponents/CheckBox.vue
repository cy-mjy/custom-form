<template>
  <div class="attr-board-container">
    ...
    <div class="attr-board-common-edge attr-board-common-edge-inline" style="justify-content:flex-start;">
      <el-radio :disabled="readonly || disableAttrs.includes('layout')" v-model="widget.layout" :label="0">平铺</el-radio>
      <el-radio :disabled="readonly || disableAttrs.includes('layout')" v-model="widget.layout" :label="1">收起</el-radio>
    </div>
    <div class="attr-board-common-title">
      <span class="attr-board-common-title-required">*</span>
      <span>选项</span>
          ...
          <div class="attr-board-option">
            <el-input :disabled="readonly || disableAttrs.includes('options')" v-model.trim="o.content" size="small" class="attr-board-option-input" :maxlength="50" @blur="onContentBlur(i)">
              <template slot="suffix">
                <span :class="{ disable: readonly || disableAttrs.includes('options') }" v-if="o.isDefault" style="cursor:pointer;color:#F54A45;" @click="cancelDefault(i)">取消默认</span>
                <span :class="{ disable: readonly || disableAttrs.includes('options') }" v-else style="cursor:pointer;color:#0b58d2;" @click="setDefault(i)">设置默认</span>
              </template>
            </el-input>
