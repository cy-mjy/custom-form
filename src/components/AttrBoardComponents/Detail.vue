<template>
  <div class="attr-board-container">
    ...
    <div class="attr-board-common-edge attr-board-common-edge-inline">
      <span>显示序号</span>
      <el-switch :disabled="readonly || disableAttrs.includes('showIndex')" v-model="widget.showIndex"></el-switch>
    </div>
    <template v-if="widget.showIndex">
      <div class="attr-board-common-title">
        <span class="attr-board-common-title-required">*</span>
        <span>序号名称</span>
      </div>
      <el-form :model="widget">
        <el-form-item
                prop="indexName"
                :rules="{
            required: true, message: '请输入序号名称', trigger: 'change'
          }"
        >
          <el-input :disabled="readonly || disableAttrs.includes('indexName')" v-model.trim="widget.indexName" size="small" :maxLength="8" placeholder="请输入序号名称" @blur="onIndexNameBlur"></el-input>
        </el-form-item>
      </el-form>
    </template>
  </div>
  ...
  methods: {
    onTipsBlur() {
      if(this.widget.buttonText === '') {
        this.widget.buttonText = '新增明细'
      }
    },
    onIndexNameBlur() {
      if(this.widget.indexName === '') {
        this.widget.indexName = '明细内容'
      }
    },
  },
}
