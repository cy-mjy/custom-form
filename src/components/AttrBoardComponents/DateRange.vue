<template>
  <div class="attr-board-container">
    ...
    <el-select :disabled="readonly || disableAttrs.includes('dateFormat')" @change="onDateFormatChange" v-model="widget.dateFormat" size="small" style="width:100%;" class="attr-board-common-edge">
...
export default {
  ...
  computed: {
    date: {
      ...
      set(val) {
        if(val === null) {
          this.widget.default.value1 = null
          this.widget.default.value2 = null
        } else {
          this.widget.default.value1 = val[0]
          this.widget.default.value2 = val[1]
        }
      },
    },
  },
  methods: {
    onLimitChange(value) {
      if(value && ((this.widget.default.value1 && this.widget.default.value1 < +Date.now()) || (this.widget.default.value2 && this.widget.default.value2 < +Date.now()))) {
        this.widget.default.value1 = null
        this.widget.default.value2 = null
      }
    },
    ...
    onTipsBlur() {
      if(this.widget.tips === '') {
        this.widget.tips = '开始时间'
      }
    },
    onTips2Blur() {
      if(this.widget.tips2 === '') {
        this.widget.tips2 = '结束时间'
      }
    },
  },
}
